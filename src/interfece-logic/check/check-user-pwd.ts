...
export function 检查密码<逻辑附加参数类型 extends {}, 字段类型 extends string>(
  字段名: 字段类型,
  ...
  return 接口逻辑.构造(插件, async (参数, 逻辑附加参数, 请求附加参数) => {
    ...
    if (参数[字段名].includes(' ')) return new Left('密码不能包含空格')
    if (参数[字段名] === '') return new Left('密码不能为空')
    if (参数[字段名].length < 6) return new Left('密码过短')
    if (参数[字段名].length > 32) return new Left('密码过长')
