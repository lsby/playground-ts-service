...
export type 集线器监听<消息类型> = {
  监听id: 集线器监听id
  发送消息: (消息: 消息类型) => Promise<void>
}

export class 集线器<消息类型> {
  private 监听列表: Map<集线器监听id, 集线器监听<消息类型>> = new Map()

  public 监听消息(监听id: string, 接收消息回调: (消息: 消息类型) => Promise<void>): 集线器监听<消息类型> {
    let 监听: 集线器监听<消息类型> = {
      监听id: 监听id,
      发送消息: async (消息: 消息类型): Promise<void> => {
        let promises: Promise<void>[] = []
        this.监听列表.forEach((当前监听, 当前监听的监听id) => {
          if (监听id === 当前监听的监听id) return
          promises.push(当前监听.发送消息(消息))
        })
  ...
  public async 广播消息(消息: 消息类型): Promise<void> {
    let promises: Promise<void>[] = []
    this.监听列表.forEach((监听) => {
      promises.push(监听.发送消息(消息))
    })
    await Promise.all(promises)
    return void 0
  }
