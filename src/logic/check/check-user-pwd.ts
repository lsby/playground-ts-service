...
function 逻辑插件<检查名称 extends string>(检查名称值: 检查名称): 逻辑插件类型<检查名称> {
  return [new Task(async () => new JSON解析插件(z.object(构造对象(检查名称值, z.string())), {}))]
}
...
> {
  ...
  override async 实现(
    ...
    if (参数[this.检查名称值].includes(' ')) return new Left('密码不能包含空格')
    if (参数[this.检查名称值] === '') return new Left('密码不能为空')
    if (参数[this.检查名称值].length < 6) return new Left('密码过短')
    if (参数[this.检查名称值].length > 32) return new Left('密码过长')
    return new Right(参数)
  }
}

export let 检查密码 = 逻辑实现
