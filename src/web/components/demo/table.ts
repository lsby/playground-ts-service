...
export class 测试表格组件 extends 组件基类<属性类型, 发出事件类型, 监听事件类型> {
  ...
  public constructor() {
    ...
    this.表格组件 = new LsbyDataTable<数据项>({
      ...
      加载数据: async (参数: 数据表加载数据参数): Promise<{ 数据: 数据项[]; 总数: number }> => {
        let { data, total } = await API管理器.请求post接口并处理错误('/api/demo/user-crud/read', {
          page: 参数.页码,
          size: 参数.每页数量,
          ...(参数.排序字段 !== null && 参数.排序字段 !== void 0 && 参数.排序方向 !== null && 参数.排序方向 !== void 0
            ? { orderBy: 参数.排序字段 as 'id' | 'name', orderDirection: 参数.排序方向 }
            : {}),
