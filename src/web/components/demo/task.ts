...
export class 测试任务组件 extends 表格组件基类<属性类型, 发出事件类型, 监听事件类型, 任务数据项> {
  ...
  private 测试任务超时时间输入框 = document.createElement('input')
  ...
  private 失败任务超时时间输入框 = document.createElement('input')
    ...
    return {
      ...
      取消: async (任务: 任务数据项): Promise<void> => {
        if (任务.状态 === '等待中' || 任务.状态 === '运行中') {
          try {
            await this.api管理器.请求接口并处理错误('/api/task-admin/cancel', { 任务id: 任务.id })
            await this.刷新任务列表()
          } catch (错误) {
  ...
  private async 刷新任务列表(): Promise<void> {
    try {
      let 结果 = await this.api管理器.请求接口并处理错误('/api/task-admin/list', {})
  ...
  private 设置创建测试任务模态框内容(): void {
    ...
    // 超时时间输入
    let 超时容器 = document.createElement('div')
    let 超时标签 = document.createElement('label')
    超时标签.textContent = '超时时间(秒):'
    超时标签.style.display = 'block'
    超时标签.style.marginBottom = '0.5em'
    this.测试任务超时时间输入框.placeholder = '可选，任务执行超时时间'
    this.测试任务超时时间输入框.type = 'number'
    this.测试任务超时时间输入框.style.width = '100%'
    this.测试任务超时时间输入框.style.padding = '0.5em'
    this.测试任务超时时间输入框.style.border = '1px solid var(--边框颜色)'
    this.测试任务超时时间输入框.style.borderRadius = '4px'
    this.测试任务超时时间输入框.value = ''
    超时容器.append(超时标签, this.测试任务超时时间输入框)
    ...
    确认按钮.onclick = async (): Promise<void> => {
      ...
      let 超时时间 = this.测试任务超时时间输入框.value.trim()
      ...
      try {
        await this.api管理器.请求接口并处理错误('/api/task-admin/create-test', {
    ...
    内容容器.append(名称容器, 消息容器, 时间容器, 超时容器, 按钮容器)
  ...
  private 设置创建失败测试任务模态框内容(): void {
    ...
    超时标签.style.display = 'block'
    超时标签.style.marginBottom = '0.5em'
    this.失败任务超时时间输入框.placeholder = '可选，任务执行超时时间'
    this.失败任务超时时间输入框.type = 'number'
    this.失败任务超时时间输入框.style.width = '100%'
    this.失败任务超时时间输入框.style.padding = '0.5em'
    this.失败任务超时时间输入框.style.border = '1px solid var(--边框颜色)'
    this.失败任务超时时间输入框.style.borderRadius = '4px'
    this.失败任务超时时间输入框.value = ''
    超时容器.append(超时标签, this.失败任务超时时间输入框)
    ...
    确认按钮.onclick = async (): Promise<void> => {
      ...
      let 超时时间 = this.失败任务超时时间输入框.value.trim()
      ...
      try {
        await this.api管理器.请求接口并处理错误('/api/task-admin/create-fail-test', {
          失败任务名称: 任务名称,
          失败消息: 失败消息,
          最大重试次数: 最大重试次数,
          任务优先级: 任务优先级,
    ...
    内容容器.append(名称容器, 消息容器, 重试容器, 延迟容器, 优先级容器, 超时容器, 按钮容器)
