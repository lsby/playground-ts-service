...
export class 环境变量管理器<环境变量描述 extends z.AnyZodObject> {
  ...
  private async 初始化(): Promise<void> {
    var log = (await GlobalLog.getInstance()).extend('env')

    await log.debug('初始化环境变量...')

    await log.debug('检查%o...', 'ENV_FILE_PATH')
    if (env['ENV_FILE_PATH'] != null) {
      var ENV_FILE_PATH = env['ENV_FILE_PATH']
      await log.debug('- 已找到%o: %o', 'ENV_FILE_PATH', ENV_FILE_PATH)
      ...
      if (fs.existsSync(ENV_FILE_PATH)) {
        ...
        await log.debug(`- 没有找到目标文件: %o`, ENV_FILE_PATH)
      }
    }
