...
function 获取目录内所有TS文件(目录: string, 根目录: string = 目录): string[] {
  ...
  for (let 文件 of 列表) {
    ...
    } else if (状态.isFile() === true && (完整路径.endsWith('.ts') === true || 完整路径.endsWith('.tsx') === true)) {
      let 相对路径 = path
        .relative(根目录, 完整路径)
        .replace(/\\/g, '/')
        .replace(/\.(ts|tsx)$/, '')
...
let 所有文件路径 = [
  ...
  f.endsWith('.ts') === true || f.endsWith('.tsx') === true
    ? f
    : f + (fs.existsSync(f + '.ts') === true ? '.ts' : '.tsx'),
)
...
let 目标组件文件 = 获取目录内所有TS文件(组件目录)
  ...
  .filter((a) => {
    ...
    let 完整路径 = fs.existsSync(路径项 + '.ts') === true ? 路径项 + '.ts' : 路径项 + '.tsx'
